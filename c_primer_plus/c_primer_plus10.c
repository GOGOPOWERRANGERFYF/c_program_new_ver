...
//  ==>注意：多个函数代码加载到内存后，指令地址几乎不可能连续,
//          例如反汇编后我们可以看到main,a,b函数连续但执行的指令不可能连续,自己反汇编看一下就明白了
//  函数的调用过程通过跳转指令实现
//  1.保存调用者将要执行的指令位置(其实也就是call被调函数的下一条指令地址),将该地址从PC中入栈
//  2.PC从IR的指令中获取被调函数的地址
//  3.被调函数执行完后，将主调函数将要执行的指令地址从栈中取回，继续执行主调函数
//  使用到汇编的两个指令: call执行1和2；ret执行3(从栈中弹出返回地址,即1中保存的地址,地址移动到PC中)
//  以上这些过程称为控制转移
//  填完这个坑又有一个新坑。。。计算机速成课讲了寄存器,内存，ALU算术逻辑单元的数字电路实现
//  但CPU控制器只讲了解码器的数字电路实现原理，整个控制器是直接抽象的,可见控制器电路灰常复杂。。。
//  所以我感觉就没必要深究了。。。又不是要设计CPU,学个编程这种程度也差不多了。。。
//  CPU控制器: IR(instruction register指令寄存器),存储指令(opcode+oprand)
//            PC(program counter程序计数器),实际是一个指令地址寄存器
//            解码器(decode)
//            连接寄存器、内存和ALU算术逻辑单元(控制中心的感觉~)
//  刚网上了解了一下，真实的CPU那是相当复杂，特别是现在的CPU。。。真的别极端较真了。。。
//  可以暂时继续搬运代码之旅了。。。20210925
//
