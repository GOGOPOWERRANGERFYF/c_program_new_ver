...
/*
    ...
    12.1.10 存储类别说明符 (未完待续...)
        关键字static和extern的含义取决于上下文.C语言有6个关键字作为存储类别说明符:auto、
        register、static、extern、_Thread_local和typedef.typedef关键字与任何内存存储
        无关,把它归于此类有一些语法上的原因(网络资料:工程使用的原因?目前暂时无法理解). ...
        ...

        auto说明符表明变量是自动存储期,只能用于块作用域的变量声明中,由于在块中声明的
...
//    该声明让变量成为只读变量.初始化后,就不能再改变它的值.
//      const int a = 12;
//    用const关键字创建不允许修改的数组:
//      const int array[5] = {1, 2, 3, 4, 5, 6};
//      
//    1.在指针和形参声明中使用const
//    声明普通变量和数组时使用const关键字很简单.指针则赋值一些,因为要区分是限定指针本身为const
//    还是限定指针指向的值为const.下面的声明:     
//      const float *pf; /* pf指向一个float类型的const值 */
//      (个人理解:pf标识符与*结合,pf为指针,指向对象,对象与float结合,对象为float类型对象,该对象再与const结合,对象为只读,
//               最后得出,pf为指向只读float类型值的指针.该类型指针本身是可修改的,可变的)
//      创建了pf指向的值不能被改变,而pt本身的值可以改变.例如,可以设置该指针指向其他const值.相比之下,下面的声明:
//      float * const pt;
//      (个人理解:pt标识符与const类型限定符结合,声明该对象为只读对象,对象与与*结合,声明该对象为指针,对象继续与float
//               结合,声明该只读指针对象指向float类型值.)
...
//      如注释所示,把const放在类型名之后、*之前,说明该指针不能用于改变它所指向的值.简而言之,const放在*左侧任意位置,
//      限定了指针指向的数据不能改变;const放在*的右侧,限定了指针本身不能改变.
//      
//      const关键字的常见用法是声明为函数形参的指针.例如...
//      (Mark==> 待续,这里的知识点要花点时间消化吸收一下...)
// 
//  12.5.2 volatile类型限定符
//  volatile 易变的;
//  volatile声明的变量防止编译器优化,不是直接从高速缓存中读取数据
//  (高速缓存中会存在数据不一致的问题,特别是多线程中,其他线程修改变量的问题
//   但volatile不具有原子性,因此处理多线程要知道锁的概念),而是从内存读取.
//    