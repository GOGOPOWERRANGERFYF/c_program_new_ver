...
/*
问题：遇到互斥锁阻塞的线程是等到时间片用完还是即刻被操作系统放入等待队列？
POE回答：
当一个线程遇到互斥锁阻塞时，它的行为取决于操作系统的调度策略。一般情况下，线程会立即被操作系统放入等待队列，而不是等待时间片用完。
当线程请求获取一个被其他线程持有的互斥锁时，如果锁已经被占用，线程将无法继续执行。此时，操作系统会将该线程置于等待状态，并将其放
入适当的等待队列中，以等待互斥锁的释放。
操作系统在等待队列中维护了一个调度机制，当互斥锁的持有者释放锁时，操作系统会从等待队列中选择一个线程，将锁分配给它，并将该线程置
于就绪状态，以便继续执行。
等待队列的调度策略可以是先进先出（FIFO）、优先级调度或其他策略，具体取决于操作系统的实现。
需要注意的是，当线程被阻塞并放入等待队列时，它会释放它当前持有的任何互斥锁，以避免死锁的发生。一旦线程被唤醒并获得锁时，它将重新
进入就绪状态，并在适当的时机被调度以继续执行。
总结而言，线程在遇到互斥锁阻塞时，通常会被操作系统立即放入等待队列，而不是等待时间片用完。操作系统会负责在互斥锁释放时唤醒等待的
线程，并按照调度策略重新安排线程的执行顺序。这样可以确保资源的正确共享和高效利用。

问题：接着上面的问题，线程放入等待队列是被动的还是主动进行系统调用实现的？
POE回答：
线程被放入等待队列是通过主动进行系统调用实现的。当一个线程遇到互斥锁阻塞时，它会主动发起系统调用，请求操作系统将其放入等待队列。
在常见的操作系统中，例如Linux或Windows，线程通常使用特定的系统调用（如pthread_cond_wait、WaitForSingleObject等）来请求
将自己置于等待状态。这些系统调用会将线程放入等待队列，并将其状态设置为等待状态，使线程暂停执行。
一旦线程被放入等待队列，它将等待特定的条件满足或者被其他线程唤醒。当互斥锁的持有者释放锁时，操作系统会根据等待队列中的调度策略
选择一个线程，将锁分配给它，并将该线程从等待状态切换为就绪状态，以便继续执行。
需要注意的是，线程的状态转换和放入等待队列的具体实现是由操作系统内核负责管理的，它们可能因操作系统的不同而有所差异。但是，无论
具体实现如何，线程被放入等待队列是通过主动进行系统调用来实现的，而不是被动的。
*/