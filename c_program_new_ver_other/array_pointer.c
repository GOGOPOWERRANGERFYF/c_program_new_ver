...
int create_array(int index_max){
    ...
    /*
        ...
        *p; *与标识符结合间接访问一个对象   
        int (*p)(); 间接访问一个对象(对象未知),以读取的部分再与()结合,结合后的整体为一个函数,所以p为指向函数的指针  
                                               ...
                                               函数，函数原型     |
                                                     |          |
        声明器：<指针；直接声明器(标识符，标识符[下标]，标识符(参数)，声明器?)；初始化内容(= 初始值)>


        储存类型 类型限定符 类型说明符| 声明器 | 更多声明器 | ;分号
        static const char p[3] = {1, 2, 3};

        (int)a + b;
        强制类型转换的个人推理：先读取圆括号里的对象,类型说明符,圆括号先让类型说明符与标识符a结合,表示a是一个int类型值
        显示类型转换完成~
   ...
   /*
        声明和定义的小小了解:
        声明相当于普通声明,它说明的并非自身,而是描述其它地方创建的"对象";   (通过函数原型来理解,只是描述函数)
        定义相当于特殊声明,它为"对象"分配内存;  (通过定义变量来理解,例如：int a = 5;)
        (注意：C的“对象”只是跟链接器有关的东西,比如函数和变量。不是面向对象里的对象)
   */
  ...
  /*
        c语言强制类型转换/显式类型转换个人理解:
            (long int *)&var
            表达式中()圆括号运算符优先级最高,先计算圆括号内的子表达式,
            这里的子表达式使用的是c语言的声明语法,左边与*间接运算符相结合的"对象"为长整型指针,
            这个例子中这个对象为&var,所以&var为长整型指针(被强制转换)

            强制转换后取原始数值,直接打印的printf函数在处理过程中数值会被转换,所以打印不出原始值
            *((long int *)&var)
  */
